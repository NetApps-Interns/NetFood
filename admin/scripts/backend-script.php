...
if(!empty($_GET['name']) && $_GET['name']=='change_password'){
    ...
    {
      $query="SELECT password FROM tbladmin WHERE password='$old'";
      $result=$conn->query($query);
      if($result->num_rows >0)
      {
         $updateq="UPDATE tbladmin SET password='$new' WHERE password='$old'";
...
// insert
if(empty($_GET['id']) && !empty($_GET['name']) && $_GET['name']=='tbladmin'){
   extract($_POST);
